...
ipcMain.handle('open-auth-window', async () => {
  ...
  // Load the auth page
  if (process.env.VITE_DEV_SERVER_URL) {
    authWindow.loadURL(`${process.env.VITE_DEV_SERVER_URL}#/auth`)
  } else {
  ...
  authWindow.on('closed', () => {
    authWindow = null
    // Notify main window that auth window closed
    if (mainWindow) {
...
ipcMain.on('auth-success', () => {
  // Close auth window and notify main window
  if (authWindow) {
    authWindow.close()
    authWindow = null
  }
